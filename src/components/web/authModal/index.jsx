...
class AuthModal extends Component {
  ...
  getFormMeta = authModalType => {
    const elementsMap = {
      ...
      register: ['account', 'password', 'confirm', 'email'],
      updateUser: ['account', 'originPassword', 'password', 'confirm', 'email']
    }
    const elements = [
      {
        key: 'account',
        label: authModalType === 'login' ? '邮箱/用户名' : '用户名',
        widget: <Input placeholder={authModalType === 'login' ? '请输入邮箱/用户名' : '请输入用户名'} />,
        rules: [
          { required: true, message: authModalType === 'login' ? 'Email/Username is required' : 'Username is required' }
        ]
      },
      {
        key: 'originPassword',
